...
func setWindowLevelViaSA(windowId: UInt32, levelKey: Int32) {
    let socketPath = "/tmp/aerospace-sa.socket"
    
    // Check if socket file exists first to avoid unnecessary socket/connect calls
    var statBuf = stat()
    if stat(socketPath, &statBuf) != 0 {
        return // Socket does not exist (SA not loaded)
    }
